...
Classical program slicing~\cite{Weiser1984,TipSurvey1995} was developed for imperative programs. The slicing criterion is a (program-point, variable) pair, and the slice is a sub-program obtained by deleting statements that preserves the runtime value of the chosen variable at the chosen point. Static and dynamic variants~\cite{KorelLaski1988} differ in whether the preservation is universally quantified over inputs or specialised to a single execution, but both share the same shape: the analysis tracks data and control dependence between statements, and the slice is a syntactic deletion that preserves a runtime observation.

Type slicing is in a different idiom on every count. The object is an expression (and assumptions) lattice rather than a sequence of statements; the criterion is a query type $\upsilon \sqsubseteq \tau$ rather than a (point, variable) pair; the preserved quantity is a synthesised type, not a runtime value; and ``deletion'' is a structural $\gap$ in the precision lattice.
