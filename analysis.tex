...
Analysis slices concern the classifications with focus mode $m = {\color{BlueViolet}\Downarrow}\tau$. A query $\upsilon \in \lfloor\tau\rfloor$ specifies the part of $\tau$ that we wish to explain. The outer derivation mode $d$ splits the construction in two; the second is a strengthening of the first, needed because some classification rules cross from synthesis-position outer to analysis-position inner (notably $s\circ_2$, the application focused on its argument).

\begin{definition}[\yo Analysis slice]
\label{def:ana-slice}
An \textit{analysis slice} of $\mathit{Cls} : \ctxclass{\C}{{\color{BrickRed}\Uparrow}\,\tau_p}{\Delta';\,\Gamma'}{{\color{BlueViolet}\Downarrow}\,\tau}$ on $\upsilon \in \lfloor\tau\rfloor$, written $\anaslice{\mathit{Cls}}{\upsilon}$, is a four-tuple
\[ (\kappa,\,\gamma) \mathbin{{\color{BlueViolet}\Downarrow}} \psi \mathbin{\sqsupseteq} \upsilon \mathbin{\in} \mathit{cls}' \]
with $(\kappa, \gamma) \in \lfloor\C, \Gamma\rfloor$, an outer-type witness $\psi \in \lfloor\tau_p\rfloor$ (by graduality, \cref{thm:graduality-syn-cls}), and $\mathit{cls}' : \ctxclass[\Delta;\,\gamma]{\kappa}{{\color{BrickRed}\Uparrow}\,\psi}{\Delta'';\,\Gamma''}{{\color{BlueViolet}\Downarrow}\,\upsilon}$.
\end{definition}

\begin{definition}[\yo Analysis position slice]
\label{def:ana-pos-slice}
An \textit{analysis position slice} of $\mathit{Cls} : \ctxclass{\C}{{\color{BlueViolet}\Downarrow}\,\tau_p}{\Delta';\,\Gamma'}{{\color{BlueViolet}\Downarrow}\,\tau}$ on $\upsilon \in \lfloor\tau\rfloor$, written $\anaposslice{\mathit{Cls}}{\upsilon}$, is a four-tuple
\[ (\kappa,\,\gamma) \mathbin{{\color{BlueViolet}\Downarrow}} \upsilon \mathbin{\Downarrow} \upsilon_{\mathrm{outer}} \mathbin{\in} \mathit{cls}' \]
with $(\kappa, \gamma) \in \lfloor\C, \Gamma\rfloor$, an outer-type witness $\upsilon_{\mathrm{outer}} \in \lfloor\tau_p\rfloor$ (chosen freely; graduality, \cref{thm:graduality-ana-cls}, ensures the classification holds for any such choice), and $\mathit{cls}' : \ctxclass[\Delta;\,\gamma]{\kappa}{{\color{BlueViolet}\Downarrow}\,\upsilon_{\mathrm{outer}}}{\Delta'';\,\Gamma''}{{\color{BlueViolet}\Downarrow}\,\upsilon}$.
\end{definition}
...
\upsilon \arrow[r, "\sqsubseteq" {sloped}] & \psi \arrow[r, induced, "\sqsubseteq" {sloped}, "\text{(by graduality, \cref{thm:graduality-syn-cls})}"', dashed] & \tau_p
\end{tikzcd}
