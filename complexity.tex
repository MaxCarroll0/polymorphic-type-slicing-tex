...
We can calculate a \textit{minimal} synthesis slice in polynomial time (TODO: ref minExists section, not the theorem). However, minimal slices are not unique, so a natural extension is to rank them by \textit{size} and ask for a minimal slice of \textit{minimum size}. This chapter reduces the \textit{set cover optimisation problem} to calculating minimum size slice, meaning minimum-size slicing is \textbf{NP-hard}. I (informally) prove this for term-minimal synthesis slices, and show that the argument extends to general minimal synthesis slices and to analysis slices.
