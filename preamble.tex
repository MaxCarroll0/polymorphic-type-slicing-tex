\usepackage[a4paper, margin=2.5cm, right=2cm]{geometry}
...
\setmonofont{Latin Modern Mono}[RawFeature={+shrinkemdash},BoldFont={lmmonolt10-bold.otf}]
...
% Slice colouring for variable references to bindings that are recursively sliced.
\newcommand{\asmref}[1]{\textcolor{assumed}{\textbf{#1}}}

% Slice text colours: soft (general slice content) vs strong (type sources).
\newcommand{\synsrc}[1]{\textcolor{BrickRed!60!gray}{#1}}
\newcommand{\anasrc}[1]{\textcolor{NavyBlue!60!gray}{#1}}
% Heavy mono for highlighted type-source spans: real Black weight, rendered as
% genuine vector glyphs (no FakeBold stroke artifacts).
\newfontfamily{\heavymono}{SourceCodePro-Black.otf}
\newcommand{\synsrcS}[1]{\textcolor{red!85!black}{{\heavymono #1}}}
\newcommand{\anasrcS}[1]{\textcolor{blue!85!black}{{\heavymono #1}}}
\newcommand{\strN}[1]{\textcolor{OliveGreen!65!black}{#1}}

% Fold glyph: dice rendered in TikZ so it does not depend on a system font with ⚂ (U+2682).
\newcommand{\fold}{%
  \tikz[baseline=0.1em]{%
    \draw[rounded corners=0.3pt, line width=0.5pt, densely dotted, teal!80!black]
...
% IDE-style red error squiggle (wavy underline beneath the argument).
% \uwave inherits the current text colour for the wave; wrap the argument in a
% \mbox with the surrounding (non-red) colour so only the underline is red.
