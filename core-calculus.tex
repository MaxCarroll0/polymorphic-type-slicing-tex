...
{\color{assumed} (\C',\; e')} \arrow[u, assumed, "\sqsubseteq" {sloped}] \arrow[r, assumed, "\text{plug}"'] & \C'[e'] \arrow[u, dashed, induced, "\sqsubseteq" {sloped}]
\end{tikzcd}
...
\[\inference[\texttt{s$\cmark$}]{}{\ctxclass{\cmark}{{\color{BrickRed}\Uparrow}\;\tau}{\Delta;\,\Gamma}{{\color{BrickRed}\Uparrow}\;\tau}} \qquad
  \inference[\texttt{a$\cmark$}]{}{\ctxclass{\cmark}{{\color{BlueViolet}\Downarrow}\;\tau}{\Delta;\,\Gamma}{{\color{BlueViolet}\Downarrow}\;\tau}}\]
...
\end{itemize}
Here ``$e$ satisfies focus mode $m$ under $\Delta';\,\Gamma'$'' means: if $m = {\color{BrickRed}\Uparrow}\;\tau'$ then $\Delta';\,\Gamma' \vdash e\;{\color{BrickRed}\Uparrow}\;\tau'$; if $m = {\color{BlueViolet}\Downarrow}\;\tau'$ then $\Delta';\,\Gamma' \vdash e\;{\color{BlueViolet}\Downarrow}\;\tau'$.
\end{theorem}
\end{majortheorembox}
...
If\/ $\Gamma' \sqsubseteq \Gamma$, $e' \sqsubseteq e$, and\/ $\synthesis{e}{\tau}$, then there exists $\tau'$ such that\/ $\Delta;\,\Gamma' \vdash e'\;{\color{BrickRed}\Uparrow}\;\tau'$ and\/ $\tau' \sqsubseteq \tau$.
\end{theorem}
\end{majortheorembox}

\begin{majortheorembox}
\begin{theorem}[\yo Static Gradual Guarantee -- Analysis]\label{thm:graduality-ana}
If\/ $\Gamma' \sqsubseteq \Gamma$, $e' \sqsubseteq e$, $\tau' \sqsubseteq \tau$, and\/ $\analysis{e}{\tau}$, then\/ $\Delta;\,\Gamma' \vdash e'\;{\color{BlueViolet}\Downarrow}\;\tau'$.
\end{theorem}
\end{majortheorembox}

The same monotonicity extends to context classifications, where $m' \sqsubseteq m$ lifts type precision through the focus mode constructors: ${\color{BrickRed}\Uparrow}\,\tau' \sqsubseteq {\color{BrickRed}\Uparrow}\,\tau$ iff $\tau' \sqsubseteq \tau$, and likewise for ${\color{BlueViolet}\Downarrow}$. The focus modes itself (analysis or synthesis) is propagated unchanged through every classification rule.

\begin{majortheorembox}
\mechbox{static-gradual-syn-cls}%
\begin{theorem}[\yo Static Gradual Guarantee -- Context, Synthesis Position]\label{thm:graduality-syn-cls}
If\/ $\Gamma' \sqsubseteq \Gamma$, $\C' \sqsubseteq \C$, and\/ $\ctxclass[\Delta;\,\Gamma]{\C}{{\color{BrickRed}\Uparrow}\,\tau_p}{\Delta_f;\,\Gamma_f}{m}$, then there exist $\tau_p'$, $\Gamma_f'$, and $m'$ with $\tau_p' \sqsubseteq \tau_p$, $\Gamma_f' \sqsubseteq \Gamma_f$, $m' \sqsubseteq m$, and\/ $\ctxclass[\Delta;\,\Gamma']{\C'}{{\color{BrickRed}\Uparrow}\,\tau_p'}{\Delta_f;\,\Gamma_f'}{m'}$.
\end{theorem}
\end{majortheorembox}

\begin{majortheorembox}
\mechbox{static-gradual-ana-cls}%
\begin{theorem}[\yo Static Gradual Guarantee -- Context, Analysis Position]\label{thm:graduality-ana-cls}
If\/ $\Gamma' \sqsubseteq \Gamma$, $\C' \sqsubseteq \C$, $\tau_p' \sqsubseteq \tau_p$, and\/ $\ctxclass[\Delta;\,\Gamma]{\C}{{\color{BlueViolet}\Downarrow}\,\tau_p}{\Delta_f;\,\Gamma_f}{m}$, then there exist $\Gamma_f'$ and $m'$ with $\Gamma_f' \sqsubseteq \Gamma_f$, $m' \sqsubseteq m$, and\/ $\ctxclass[\Delta;\,\Gamma']{\C'}{{\color{BlueViolet}\Downarrow}\,\tau_p'}{\Delta_f;\,\Gamma_f'}{m'}$.
\end{theorem}
\end{majortheorembox}
...
If\/ $\Gamma' \sqsubseteq \Gamma$, $e' \sqsubseteq e$, $\synthesis{e}{\tau}$, and\/ $\Delta;\,\Gamma' \vdash e'\;{\color{BrickRed}\Uparrow}\;\tau'$, then $\tau' \sqsubseteq \tau$.
\end{corollary}

\paragraph{Mechanisation provenance.} The Agda mechanisation of graduality on expression typing (\cref{thm:graduality-syn,thm:graduality-ana}) is loosely based on
\href{https://github.com/hazelgrove/hazelnut-polymorphism-agda}{\texttt{hazelgrove/hazelnut-polymorphism-agda}}, but had to be re-proven from scratch to integrate with the Type Slicing core calculus.\footnote{Which is sufficiently similar to Hazel for this provenance to be useful.} The graduality theorems for context classification (\cref{thm:graduality-syn-cls,thm:graduality-ana-cls}) are new to this work, since context classification itself is novel.
